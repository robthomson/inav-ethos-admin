...
local function setLogging(level, opts)
    ...
    inavadmin.utils.log(_mspPrefix .. "logging set to level=" .. tostring(_mspLogLevel) ..
...
local function _log(level, msg)
    if _mspLogLevel >= level then
        inavadmin.utils.log(_mspPrefix .. msg,"info")
    end
end
...
local function _hex(buf, maxLen)
    ...
    inavadmin.utils.log(_mspPrefix .. "HEX[" .. tostring(n) .. "]: " .. table.concat(t, " "), "debug")
    if #buf > n then
        inavadmin.utils.log(_mspPrefix .. "(+" .. tostring(#buf - n) .. " more bytes)", "debug")
    end
end
-- ============================================================================
...
function handlers.v1.processTxQ()
    ...
    if i <= inavadmin.tasks.msp.protocol.maxTxBufferSize then
        ...
        _hex(payload, inavadmin.tasks.msp.protocol.maxTxBufferSize)
...
function handlers.v1.sendRequest(cmd, payload)
    ...
    _hex(mspTxBuf, inavadmin.tasks.msp.protocol.maxTxBufferSize)
end
...
function handlers.v1.receivedReply(payload)
    ...
    _hex(mspRxBuf, inavadmin.tasks.msp.protocol.maxRxBufferSize)
...
function handlers.v2.processTxQ()
    ...
    for j = i, inavadmin.tasks.msp.protocol.maxTxBufferSize do
