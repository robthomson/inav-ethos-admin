...
local function applySagCompensation(voltage)
    if inavsuite.flightmode.current ~= "inflight" then return voltage end
...
local function smartFuelCalc()
    if not telemetry then telemetry = inavsuite.tasks.telemetry end
    ...
    local now = os.clock()
    if (inavsuite.flightmode.current == "inflight" or inavsuite.flightmode.current == "postflight") and lastFuelPercent and lastFuelTimestamp then
