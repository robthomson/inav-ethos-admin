...
local useRawValue = inavsuite.utils.ethosVersionAtLeast({1, 7, 0})
...
local sidLookup = {
  ...
  [3]  = {'0x1011'}, -- Battery voltage
  [4]  = {'0x1012'}, -- Battery current
  [5]  = {'0x1013'}, -- Battery consumption
  [6]  = {'0x1014'}, -- Battery charge level
  [7]  = {'0x1020'}, -- Battery cell count
  [8]  = {'0x1021'}, -- Battery cell voltage
  [9]  = {'0x102F'}, -- Battery all cell voltages

  [11] = {'0x10B1'}, -- Heading
  [12] = {'0x10B2'}, -- Altitude
  [13] = {'0x10B3'}, -- Variometer

  [14] = {'0x1100'}, -- Attitude (packed)
  [15] = {'0x1101'}, -- Attitude pitch
  [16] = {'0x1102'}, -- Attitude roll
  [17] = {'0x1103'}, -- Attitude yaw
  ...
  [23] = {'0x1123'}, -- GPS HDOP
  [24] = {'0x1125'}, -- GPS coord
  [25] = {'0x1126'}, -- GPS altitude
  [26] = {'0x1127'}, -- GPS heading
  [27] = {'0x1128'}, -- GPS groundspeed
  [28] = {'0x1129'}, -- GPS home distance
  [29] = {'0x112A'}, -- GPS home direction
  [30] = {'0x112B'}, -- GPS azimuth (make sure sensorsList has 0x112B if you enable this)

  [31] = {'0x1131'}, -- ESC RPM (aggregate)
  [33] = {'0x1132'}, -- ESC1 RPM
  [35] = {'0x1133'}, -- ESC2 RPM
  [37] = {'0x1134'}, -- ESC3 RPM
  [39] = {'0x1135'}, -- ESC4 RPM

  [32] = {'0x1136'}, -- ESC temp (aggregate)
  [34] = {'0x1137'}, -- ESC1 temp
  [36] = {'0x1138'}, -- ESC2 temp
  [38] = {'0x1139'}, -- ESC3 temp
  [40] = {'0x113A'}, -- ESC4 temp
  ...
  [42] = {'0x1150'}, -- CPU load
  [43] = {'0x1251'}, -- Flight mode
  [44] = {'0x1253'}, -- Profiles
  [45] = {'0x1252'}, -- Arming flags
}
...
local function createTelemetrySensor(uid, name, unit, dec, value, min, max)
    ...
    if inavsuite.session.telemetryState == false then return end
...
local function setTelemetryValue(uid, subid, instance, value, unit, dec, name, min, max)
    ...
    if sensors['uid'][uid] == nil then
        ...
        if sensors['uid'][uid] == nil then
            if inavsuite.utils and inavsuite.utils.log then inavsuite.utils.log("Create sensor: " .. tostring(uid), "debug") end
...
local sensorsList = {
    ...
    -- ESC (generic + indexed)
    [0x1131] = {name = "ESC RPM", unit = UNIT_RPM, prec = 0, min = 0, max = 65535, dec = decEscRpm},
    [0x1132] = {name = "ESC1 RPM", unit = UNIT_RPM, prec = 0, min = 0, max = 65535, dec = decU16},
    [0x1133] = {name = "ESC2 RPM", unit = UNIT_RPM, prec = 0, min = 0, max = 65535, dec = decU16},
    [0x1134] = {name = "ESC3 RPM", unit = UNIT_RPM, prec = 0, min = 0, max = 65535, dec = decU16},
    [0x1135] = {name = "ESC4 RPM", unit = UNIT_RPM, prec = 0, min = 0, max = 65535, dec = decU16},

    [0x1136] = {name = "ESC Temp", unit = UNIT_CELSIUS, prec = 0, min = 0, max = 255, dec = decEscTemp},
    [0x1137] = {name = "ESC1 Temp", unit = UNIT_CELSIUS, prec = 0, min = 0, max = 255, dec = decU8},
    [0x1138] = {name = "ESC2 Temp", unit = UNIT_CELSIUS, prec = 0, min = 0, max = 255, dec = decU8},
    [0x1139] = {name = "ESC3 Temp", unit = UNIT_CELSIUS, prec = 0, min = 0, max = 255, dec = decU8},
    [0x113A] = {name = "ESC4 Temp", unit = UNIT_CELSIUS, prec = 0, min = 0, max = 255, dec = decU8},

    -- System
    [0x1150] = {name = "CPU Load", unit = UNIT_PERCENT, prec = 0, min = 0, max = 100, dec = decU8},

    -- Flight state
    [0x1251] = {name = "Flight Mode", unit = UNIT_RAW, prec = 0, min = 0, max = 65535, dec = decU16},
    [0x1252] = {name = "Arming Flags", unit = UNIT_RAW, prec = 0, min = 0, max = 255, dec = decU8},
    [0x1253] = {name = "Profiles", unit = UNIT_RAW, prec = 0, min = 0, max = 65535, dec = decU16},
}
...
function elrs.crossfirePop()
    ...
    if (inavsuite.session.telemetryState == false) then
        local module = model.getModule(inavsuite.session.telemetrySensor:module())
        if module ~= nil and module.muteSensorLost ~= nil then module:muteSensorLost(5.0) end

        if inavsuite.session.telemetryState == false then resetSensors() end
...
function elrs.wakeup()
    ...
    if inavsuite.session.telemetryState and inavsuite.session.telemetrySensor then
